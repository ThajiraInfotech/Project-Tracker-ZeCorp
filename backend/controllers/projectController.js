...
exports.createProject = async (req, res) => {
  try {
    const { projectName, projectType, category, jobOrder, outlet, description, clientName, clientEmail, clientPhone, startDate, endDate, budget, location, manager, teamMembers } = req.body;
...
exports.getOutlets = async (req, res) => {
  try {
    const outlets = await Project.distinct('outlet');
    res.json({
      success: true,
      outlets: outlets.filter(Boolean).sort() // Remove null/undefined and sort
    });
  } catch (error) {
    console.error('Get outlets error:', error);
    res.status(500).json({ message: 'Failed to get outlets' });
  }
};
