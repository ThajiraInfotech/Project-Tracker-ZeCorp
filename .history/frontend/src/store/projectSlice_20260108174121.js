...
export const addProjectDiscussion = createAsyncThunk(
  ...
  async ({ projectId, content, system, parentDiscussionId }, { rejectWithValue }) => {
    try {
      const response = await api.post(`/projects/${projectId}/discussions`, { content, system, parentDiscussionId });
...
// Async thunk for replying to a project discussion
export const replyToProjectDiscussion = createAsyncThunk(
  'projects/replyToProjectDiscussion',
  async ({ projectId, parentDiscussionId, content, system }, { rejectWithValue }) => {
    try {
      const response = await api.post(`/projects/${projectId}/discussions`, {
        content,
        system: system || false,
        parentDiscussionId
      });
...
const projectSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(replyToProjectDiscussion.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(replyToProjectDiscussion.fulfilled, (state, action) => {
        state.loading = false;
        const { projectId, discussion } = action.payload;
        if (!state.projectDiscussions[projectId]) {
          state.projectDiscussions[projectId] = [];
      ...
      .addCase(replyToProjectDiscussion.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
