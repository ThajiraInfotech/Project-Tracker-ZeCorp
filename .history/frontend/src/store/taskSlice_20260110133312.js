...
export const addTaskDiscussion = createAsyncThunk(
  ...
  async ({ taskId, content, system, parentDiscussionId, attachments }, { rejectWithValue }) => {
    try {
      const formData = new FormData();
      formData.append('content', content);
      if (system) formData.append('system', system);
      if (parentDiscussionId) formData.append('parentDiscussionId', parentDiscussionId);
      ...
      const response = await api.post(`/tasks/${taskId}/discussions`, formData, {
        headers: {
...
export const replyToTaskDiscussion = createAsyncThunk(
  ...
  async ({ taskId, parentDiscussionId, content, system, attachments }, { rejectWithValue }) => {
    try {
      const formData = new FormData();
      formData.append('content', content);
      formData.append('system', system || false);
      formData.append('parentDiscussionId', parentDiscussionId);
      if (attachments && attachments.length > 0) {
        Array.from(attachments).forEach((file, index) => {
          formData.append('attachments', file);
        });
