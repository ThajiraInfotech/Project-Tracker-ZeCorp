...
const ManagerDashboard = () => {
  ...
  const fetchManagerData = async () => {
    try {
      ...
      const projectsResponse = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/projects`, { headers });
      ...
      const tasksResponse = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/tasks`, { headers });
      ...
      const staffResponse = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/auth/staff-for-manager`, { headers });
  ...
  const handleCreateTask = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Refresh tasks
        const tasksResponse = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/tasks`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const tasksData = tasksResponse.data.tasks || tasksResponse.data || [];
        setTasks(Array.isArray(tasksData) ? tasksData : []);
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {staff.map((member) => {
                ...
                return (
                  <div key={member._id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="bg-white rounded-xl max-w-md w-full mx-4">
              <div className="px-6 py-4 border-b border-gray-200">
                <h3 className="text-lg font-semibold text-gray-900">
                  {selectedProject ? `Create Task for ${selectedProject.projectName}` : 'Create New Task'}
                </h3>
              </div>
