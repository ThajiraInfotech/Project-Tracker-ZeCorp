...
const StaffDashboard = () => {
  ...
  useEffect(() => {
    const fetchStaffData = async () => {
      try {
        ...
        const staffTasks = allTasks.filter(task => task.assignedTo?.id === user.id || task.assignedTo === user.id);
        setTasks(staffTasks);
        ...
        const projectIds = staffTasks.map(task => task.project?._id || task.project).filter(Boolean);
        ...
        const attendanceResponse = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/attendance`, { headers });
  ...
  const updateTaskProgress = async (taskId, newProgress) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/tasks/${taskId}`,
