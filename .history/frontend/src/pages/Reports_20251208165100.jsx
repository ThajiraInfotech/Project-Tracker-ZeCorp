...
const Reports = () => {
  ...
  const fetchReports = async () => {
    try {
      ...
      if (reportType === 'dashboard') {
        ...
        if (response.payload) {
          // Dashboard data is already handled by the slice
        }
      }
      // Fetch project reports
      else if (reportType === 'projects') {
        const response = await axios.get('http://localhost:5000/api/reports/admin/project-performance', {
          headers: {
        ...
        if (response.data.success && response.data.report) {
          setProjectReports(response.data.report);
        }
      }
      // Fetch task reports - use daily report which includes tasks
        ...
        if (response.data.success && response.data.report) {
          setTaskReports(response.data.report.tasksCompleted);
        }
      }
      // Fetch attendance reports - use daily report which includes attendance
        ...
        if (response.data.success && response.data.report) {
          setAttendanceReports(response.data.report.attendance);
        }
      }
  ...
  }, [auth.isAuthenticated, reportType]);
  ...
  const AttendanceReportCard = ({ record }) => (
    <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow duration-300">
      <div className="flex justify-between items-start mb-4">
        <h3 className="text-lg font-semibold text-gray-900">{record.user?.fullName}</h3>
