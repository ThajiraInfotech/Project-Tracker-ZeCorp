...
exports.addDiscussion = async (req, res) => {
  try {
    const { content, system, parentDiscussionId } = req.body;
    
    console.log('addDiscussion called with:', { content: typeof content, parentDiscussionId, files: req.files?.length });
    ...
    if (parentDiscussionId) {
      const parentExists = task.discussions.some(d => d._id.toString() === parentDiscussionId.toString());
      if (!parentExists) {
    ...
    task.discussions.push(newDiscussion);

    await task.save();

    // Create mention notifications (optional - don't fail if notifications fail)
    try {
      await createMentionNotifications(mentionedUsers, req.user, 'Task', task._id, content);
    ...
    res.status(500).json({ message: 'Failed to add discussion', error: error.message });
  }
};

// Get task discussions with threading
exports.getDiscussions = async (req, res) => {
  try {
    ...
    const sortedDiscussions = task.discussions.sort((a, b) => {
...
// Get all task discussions for the current user
exports.getAllTaskDiscussions = async (req, res) => {
  try {
    let taskQuery = {};

    // Filter by user role
    if (req.user.role === 'staff') {
      taskQuery.assignedTo = req.user._id.toString();
    } else if (req.user.role === 'manager') {
      // Get projects managed by this user
      const managedProjects = await Project.find({ manager: req.user._id.toString() });
      const projectIds = managedProjects.map(project => project._id);
    ...
    // Admin sees all task discussions (no filter)

    // Get all tasks with discussions
    const tasks = await Task.find(taskQuery)
      .populate('discussions.author', 'username fullName profileImage')
      .populate('project', 'projectName')
      .populate('assignedTo', 'username fullName');

    // Flatten all discussions from all tasks
    let allDiscussions = [];
    tasks.forEach(task => {
      if (task.discussions && task.discussions.length > 0) {
        const taskDiscussions = task.discussions.map(discussion => ({
          ...discussion._doc,
          taskId: task._id,
          taskTitle: task.title,
          taskProject: task.project?.projectName || 'Unknown',
          taskAssignedTo: task.assignedTo?.fullName || 'Unknown'
        }));
        allDiscussions = allDiscussions.concat(taskDiscussions);
      }
    });
    ...
    console.error('Get all task discussions error:', error);
    res.status(500).json({ message: 'Failed to get task discussions' });
  }
};
