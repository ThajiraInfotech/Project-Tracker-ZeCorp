...
exports.createProject = async (req, res) => {
  try {
    ...
    // Add system message for project creation
    project.discussions.push({
      user: req.user._id,
      text: `Project created by ${req.user.fullName || req.user.username}`,
      messageType: 'system'
    });
...
exports.getProjectById = async (req, res) => {
  try {
    const project = await Project.findById(req.params.id)
      ...
      .populate('teamMembers', 'username fullName email')
      .populate('createdBy', 'username fullName email')
      .populate('discussions.user', 'username fullName profileImage')
      .populate('discussions.mentions', 'username fullName')
      .populate('discussions.parentDiscussion');
    ...
    // Sort discussions by createdAt
    if (project.discussions) {
      project.discussions.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
    }
...
exports.updateProject = async (req, res) => {
  try {
    ...
    // Add system message for project update
    project.discussions.push({
      user: req.user._id,
      text: `Project updated by ${req.user.fullName || req.user.username}`,
      messageType: 'system'
    });
...
exports.addTeamMember = async (req, res) => {
  try {
    ...
    // Add system message for team member addition
    project.discussions.push({
      user: req.user._id,
      text: `Team member ${user.fullName} added by ${req.user.fullName || req.user.username}`,
      messageType: 'system'
    });
...
exports.removeTeamMember = async (req, res) => {
  try {
    ...
    // Add system message for team member removal
    const removedUser = await User.findById(userId);
    project.discussions.push({
      user: req.user._id,
      text: `Team member ${removedUser?.fullName || 'Unknown'} removed by ${req.user.fullName || req.user.username}`,
      messageType: 'system'
    });
...
exports.addDiscussion = async (req, res) => {
  try {
    const { text, parentDiscussion, attachments } = req.body;
      ...
      if (user) {
        ...
        try {
          await emailService.sendEmail(
            ...
            `<p>You were mentioned in the discussion for project: ${project.projectName}</p>
             <p>By: ${req.user.fullName || req.user.username}</p>
             <p>Message: ${text}</p>`
          );
        } catch (emailError) {
          console.warn('Failed to send mention notification:', emailError.message);
        }
      }
    }
    ...
    const newDiscussion = {
      ...
      text,
      messageType: 'user',
      parentDiscussion: parentDiscussion || null,
      mentions,
    ...
    const addedDiscussion = populatedProject.discussions[populatedProject.discussions.length - 1];

    // Emit real-time update
    if (io) {
      io.to(`project-${req.params.id}`).emit('newDiscussion', { ...addedDiscussion.toObject(), projectId: req.params.id });
    }

    res.json({
      success: true,
      message: 'Discussion added successfully',
      discussion: addedDiscussion
    });
  } catch (error) {
...
exports.getDiscussions = async (req, res) => {
  try {
    ...
    // Sort discussions by createdAt
    const sortedDiscussions = project.discussions.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
