...
exports.getTeamAttendance = async (req, res) => {
  try {
    ...
    console.log('Staff from tasks:', staffIds.length);

    // Also include teamMembers from projects
    const teamMembers = projects.flatMap(p => p.teamMembers || []);
    console.log('Team members from projects:', teamMembers.length);

    const allStaffIds = [...new Set([...staffIds, ...teamMembers])];
    console.log('Total unique staff IDs:', allStaffIds.length);
    ...
    res.status(500).json({ message: 'Failed to get team attendance', error: error.message });
  }
};
