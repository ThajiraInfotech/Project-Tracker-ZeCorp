...
exports.createTask = async (req, res) => {
  try {
    ...
    task.discussions.push({
      user: req.user._id,
      text: `Task created by ${req.user.fullName || req.user.username}`,
      messageType: 'system'
    });
...
exports.getTaskById = async (req, res) => {
  try {
    ...
    if (task.discussions) {
      task.discussions.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
    }
...
exports.updateTaskStatus = async (req, res) => {
  try {
    ...
    if (oldStatus !== status) {
      task.discussions.push({
        user: req.user._id,
        text: `Status changed from ${oldStatus} to ${status} by ${req.user.fullName || req.user.username}`,
        messageType: 'system'
      });
    }
...
exports.addDiscussion = async (req, res) => {
  try {
      ...
      if (user) {
        ...
        try {
          await emailService.sendEmail(
            ...
            `<p>You were mentioned in the discussion for task: ${task.title}</p>
    ...
    const addedDiscussion = populatedTask.discussions[populatedTask.discussions.length - 1];

    // Emit real-time update
    if (io) {
      io.to(`task-${req.params.id}`).emit('newDiscussion', { ...addedDiscussion.toObject(), taskId: req.params.id });
    }
...
exports.getDiscussions = async (req, res) => {
  try {
    ...
    // Sort discussions by createdAt
    const sortedDiscussions = task.discussions.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
