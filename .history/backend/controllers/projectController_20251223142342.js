...
exports.createProject = async (req, res) => {
  try {
    ...
    // Log activity
    await logActivity(project._id, req.user._id, 'project_created', `Project "${projectName}" was created`);
...
exports.updateProject = async (req, res) => {
  try {
    ...
    // Log activity
    await logActivity(project._id, req.user._id, 'project_updated', `Project "${project.projectName}" was updated`);
...
exports.addTeamMember = async (req, res) => {
  try {
    ...
    await logActivity(project._id, req.user._id, 'team_member_added', `User "${addedUser.fullName || addedUser.username}" was added to the project team`);
...
exports.removeTeamMember = async (req, res) => {
  try {
    ...
    await logActivity(project._id, req.user._id, 'team_member_removed', `User "${removedUser.fullName || removedUser.username}" was removed from the project team`);
...
exports.getProjectActivities = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to get project activities' });
  }
};
...
exports.adminAssignManager = async (req, res) => {
  try {
    ...
    // Log activity
    await logActivity(project._id, req.user._id, 'manager_assigned', `Manager "${manager.username}" was assigned to the project by admin`);
