...
const { parseMentions, createMentionNotifications } = require('../utils/mentionUtils');
...
exports.getAllProjects = async (req, res) => {
  try {
    ...
    const projects = await Project.find(query)
      ...
      .populate('discussions.author', 'username fullName profileImage')
      .sort({ createdAt: -1 });
...
exports.getProjectFiles = async (req, res) => {
  try {
    ...
    if (!isAuthorized) {
      console.warn('Unauthorized access attempt to project:', req.params.id, 'by user:', req.user._id);
      return res.status(403).json({ message: 'Access denied' });
    }
    
    console.log('User authorized to add discussion to project:', req.params.id);
  ...
  try {
    ...
    const projectsWithAnalytics = await Promise.all(projects.map(async (project) => {
      ...
      return {
        ...project._doc,
        analytics: {
          taskCompletionRate: project.progress || 0,
          overdueTasks,
          highPriorityTasks,
          ...
          riskLevel,
          revenueStatus,
          budgetUtilization: project.budget ? `${project.progress || 0}%` : 'N/A'
        }
      };
    }));
...
exports.adminMarkProjectDelayed = async (req, res) => {
  try {
    ...
    // Send notifications to team (optional - don't fail if email fails)
    try {
      ...
      console.warn('Failed to send project delay notification emails:', emailError.message);
      // Don't fail project delay marking due to email error
    }
...
exports.addDiscussion = async (req, res) => {
  try {
    ...
    // Handle both regular JSON and FormData requests
    let content, system, parentDiscussionId;
    
    // For FormData, the fields might be nested or in different format
    if (req.body && typeof req.body === 'object') {
      // Try to get content from different possible field names
      content = req.body.content || req.body.discussionContent || req.body.text;
      system = req.body.system || req.body.isSystem || false;
      parentDiscussionId = req.body.parentDiscussionId || req.body.parentId;
    }
    
    console.log('Parsed discussion data:', { content, system, parentDiscussionId, fileCount: req.files?.length });
    ...
    // Validate parentDiscussionId if provided
    if (parentDiscussionId) {
      const parentExists = project.discussions.some(d => d._id.toString() === parentDiscussionId.toString());
      if (!parentExists) {
        return res.status(400).json({ message: 'Parent discussion not found' });
      }

      // Prevent circular references - check if parentDiscussionId is trying to reply to itself
    ...
    if (req.files && req.files.length > 0) {
      try {
        ...
        attachments = fileUrls.map((url, index) => {
          ...
          return {
            ...
            fileType: originalFile.mimetype.split('/')[1] || 'unknown',
            uploadedAt: new Date()
          };
        });
        
        console.log('Successfully uploaded attachments:', attachments);
      } catch (uploadError) {
      ...
      console.log('No files attached to discussion');
    }
    ...
    newDiscussion.mentions = mentionedUsers.map(u => u.userId);

    project.discussions.push(newDiscussion);

    await project.save();

    // Create mention notifications (optional - don't fail if notifications fail)
    try {
      await createMentionNotifications(mentionedUsers, req.user, 'Project', project._id, content);
    } catch (notificationError) {
      console.warn('Failed to create mention notifications:', notificationError.message);
      // Don't fail discussion creation due to notification error
    }

    console.log('Discussion added successfully. Project now has', project.discussions.length, 'discussions');
...
// Get project discussions with threading
exports.getDiscussions = async (req, res) => {
  try {
    ...
    // Sort discussions: top-level first, then replies in chronological order
    const sortedDiscussions = project.discussions.sort((a, b) => {
      ...
      // If both are replies or both are top-level, sort by creation date
