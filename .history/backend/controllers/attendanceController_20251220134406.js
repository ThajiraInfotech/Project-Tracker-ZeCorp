...
exports.getTeamAttendance = async (req, res) => {
  try {
    ...
    // Get staff from teamMembers of these projects
    const staffIds = projects.flatMap(p => p.teamMembers || []).filter(Boolean);
    const uniqueStaffIds = [...new Set(staffIds.map(id => id.toString()))];

    // Get today's attendance for team members
    const today = new Date().toISOString().split('T')[0];
    const attendance = await Attendance.find({
      userId: { $in: uniqueStaffIds },
      date: today
