    ...
    res.json({
      ...
      dashboard: {
        ...
        tasks: {
          totalTasksCompleted,
          totalTasksOverdue,
          tasksDueToday: incompleteTasks.filter(t => new Date(t.deadline) >= startDate && new Date(t.deadline) <= endDate && new Date(t.deadline) >= now).length,
          tasksInProgress: incompleteTasks.filter(t => new Date(t.deadline) > now && !(new Date(t.deadline) >= startDate && new Date(t.deadline) <= endDate)).length
        },
        projects: {
          totalProjects,
...
exports.getAdminDashboardData = async (req, res) => {
  try {
    ...
    const totalTasksOverdue = incompleteTasks.filter(t => new Date(t.deadline) < now).length;
    const totalProjects = projects.length;
    const totalProjectsInProgress = projects.filter(p => p.status === 'in-progress').length;
    const totalProjectsCompleted = projects.filter(p => p.status === 'completed').length;
    const totalProjectsDelayed = projects.filter(p => p.status === 'on-hold' || (p.endDate < now && p.status !== 'completed')).length;
    ...
    const delayedProjects = projects.filter(p =>
      p.status === 'on-hold' ||
      (p.endDate < now && p.status !== 'completed') ||
      p.status === 'delayed'
    );
    ...
    res.json({
      ...
      dashboard: {
        ...
        delayedProjects: delayedProjects.map(p => ({
          ...
          name: p.projectName,
          status: p.status,
          delayReason: p.endDate < now ? 'Past deadline' : 'On hold',
          daysDelayed: p.endDate < now ? Math.floor((now - p.endDate) / (1000 * 60 * 60 * 24)) : 0,
