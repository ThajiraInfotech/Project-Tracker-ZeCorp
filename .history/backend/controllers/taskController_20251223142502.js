...
exports.updateTask = async (req, res) => {
  try {
    ...
    // Log activity
    await logActivity(task.project, req.user._id, 'task_updated', `Task "${task.title}" was updated`);
...
exports.updateTaskStatus = async (req, res) => {
  try {
    ...
    // Log activity
    if (status === 'completed') {
      await logActivity(task.project, req.user._id, 'task_completed', `Task "${task.title}" was completed`);
    } else {
      await logActivity(task.project, req.user._id, 'task_updated', `Task "${task.title}" was updated`);
    }
...
exports.updateTaskProgress = async (req, res) => {
  try {
    ...
    // Log activity
    await logActivity(task.project, req.user._id, 'task_updated', `Task "${task.title}" status changed to ${status}`);
...
exports.updateTaskStatusAndProgress = async (req, res) => {
  try {
    ...
    // Log activity
    await logActivity(task.project, req.user._id, 'task_created', `Task "${task.title}" was created and assigned to ${userExists.fullName || userExists.username}`);
