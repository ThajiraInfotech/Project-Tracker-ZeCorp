...
exports.getDailyReport = async (req, res) => {
  try {
    ...
    // Convert dates to string format for attendance query
    const reportDateStr = reportDate.toISOString().split('T')[0];
    const endDateStr = endDate.toISOString().split('T')[0];

    let attendanceQuery = { date: { $gte: reportDateStr, $lte: endDateStr } };
...
exports.getWeeklyReport = async (req, res) => {
  try {
    ...
    let attendanceQuery = { date: { $gte: startDateStr, $lte: endDateStr } };
...
exports.getUserDailyReport = async (req, res) => {
  try {
    ...
    const attendance = await Attendance.findOne({
      userId: userId,
      date: { $gte: reportDateStr, $lte: endDateStr }
    });
...
exports.getDashboardData = async (req, res) => {
  try {
    ...
    // Convert dates to string format for attendance query (date field is string YYYY-MM-DD)
    ...
    // Get tasks data - include overdue tasks, future tasks, and completed tasks in period
    const tasks = await Task.find({
      $or: [
        { deadline: { $gte: today } }, // Tasks due from today onwards
        { completionDate: { $gte: today } }, // Tasks completed from today onwards
        { status: { $ne: 'completed' }, deadline: { $lt: new Date() } } // Overdue tasks (deadline < now and not completed)
      ]
    }).populate('project', 'projectName');
    ...
    const productivityPercentage = periodTasks.length > 0 ?
      Math.round((periodTasks.filter(t => t.status === 'completed').length / periodTasks.length) * 100) : 0;
    ...
    res.json({
      ...
      dashboard: {
        ...
        tasks: {
          totalTasksCompleted,
          totalTasksOverdue,
          tasksDueToday: incompleteTasks.filter(t => t.deadline && new Date(t.deadline) >= today && new Date(t.deadline) <= endDate && new Date(t.deadline) >= now).length,
          tasksInProgress: incompleteTasks.filter(t => t.deadline && new Date(t.deadline) > now && !(new Date(t.deadline) >= today && new Date(t.deadline) <= endDate)).length
        },
        projects: {
          totalProjects,
...
exports.getManagerDashboardData = async (req, res) => {
  try {
    ...
    const attendance = await Attendance.find({
      userId: { $in: managedProjects.flatMap(p => [p.manager, ...p.teamMembers]) },
      date: { $gte: todayStr, $lte: endDateStr }
...
exports.getAdminDashboardData = async (req, res) => {
  try {
    ...
    // Get tasks data - get all incomplete tasks for overdue/due calculations
    ...
    res.json({
      ...
      dashboard: {
        ...
        tasks: {
          totalTasksCompleted,
          totalTasksOverdue,
          tasksDueToday: incompleteTasks.filter(t => t.deadline && new Date(t.deadline) >= startDate && new Date(t.deadline) <= endDate && new Date(t.deadline) >= now).length,
          tasksInProgress: incompleteTasks.filter(t => t.deadline && new Date(t.deadline) > now && !(new Date(t.deadline) >= startDate && new Date(t.deadline) <= endDate)).length
        },
        projects: {
          totalProjects,
          totalProjectsInProgress,
  ...
  try {
    ...
    if (month && year) {
      ...
      attendanceRecords = await Attendance.find({
        date: { $gte: startDateStr, $lte: endDateStr }
      }).populate('userId', 'username fullName department');

      reportPeriod = { month: reportMonth + 1, year: reportYear };
    } else {
      attendanceRecords = await Attendance.find({}).populate('user', 'username fullName department');
    ...
    const attendanceSummary = users.map(user => {
      const userRecords = attendanceRecords.filter(r => r.userId && r.userId._id.toString() === user._id.toString());
