...
class EmailService {
  ...
  async sendMentionNotificationEmail(userEmail, mentionData) {
    const subject = `You were mentioned by ${mentionData.senderName}`;
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #2c3e50;">You Were Mentioned</h2>
        <p>Hello,</p>
        <p><strong>${mentionData.senderName}</strong> mentioned you in a ${mentionData.contextType}:</p>

        <div style="background: #f8f9fa; padding: 15px; border-radius: 5px; margin: 15px 0; border-left: 4px solid #007bff;">
          <p style="margin: 0;">${mentionData.content}</p>
        </div>

        <p><a href="#" style="background: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">View ${mentionData.contextType}</a></p>
        
        <p>Best regards,<br/>Thajira WorkFlow Team</p>
      </div>
    `;
