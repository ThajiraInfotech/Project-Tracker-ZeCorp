...
async function createTestData() {
  ...
  const testAttendance = new Attendance({
    ...
    location: {
      type: 'Point',
      coordinates: [0, 0] // Default coordinates for testing
    },
...
async function testAdminDashboard() {
  ...
  // Check if staff property exists before accessing it
  if (dashboard.staff) {
    console.log(`👥 Staff: ${dashboard.staff.totalStaffCount} total, ${dashboard.staff.activeStaffCount} active`);
  } else {
    console.log('ℹ️  Staff data not available in this dashboard view');
  }
